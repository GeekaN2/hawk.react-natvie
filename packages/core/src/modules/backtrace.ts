...
export default class BacktraceHelper {
   ...
   * @returns {BacktraceFrame}
   */
  private parseStackFrame(frame: StackFrame): BacktraceFrame {
     ...
     */
    if (!this.isInternal(frame) && frame.getFileName() && frame.getLineNumber()) {
      sourceCode = this.getFileChunk(frame.getFileName(), frame.getLineNumber());
    }
   ...
   * @returns {boolean}
   */
  private isInternal(frame: StackFrame): boolean {
    const hasEntriesForNonInternalCalls = frame.getFileName() &&
      frame.getFileName().indexOf(':\\') !== 1 &&
      !frame.getFileName().startsWith('.') &&
      !frame.getFileName().startsWith('/');

    return frame.isNative() || !!hasEntriesForNonInternalCalls;
  }
